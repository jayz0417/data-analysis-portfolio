...
WITH base AS (
  ...
  WHERE r.state IN (3) AND r.member_imaginary IN (0, 9) AND z.region1 = '제주특별자치도'
...
base_calc AS (
  ...
  CASE WHEN datetime_diff(end_at, start_at, hour) >= 10 THEN '장시간'
       WHEN datetime_diff(end_at, start_at, hour) < 10 THEN '단시간' END as r_type4, ## 장/단시간
...
WHERE date(r.end_at, 'Asia/Seoul') BETWEEN current_date('Asia/Seoul') -32 AND current_date('Asia/Seoul')-2
                                  AND z.region1 IN ('제주특별자치도')
                                  AND r.state IN (3) AND r.member_imaginary IN (0, 9)
                                  AND r.zone_id NOT IN (105, 9890, 13858)
)
...
SELECT
  ...
  ## 사전예약률
  count(CASE WHEN r_type3 = 'r_7dayO' THEN member_id END) + count(CASE WHEN r_type3 = 'r_7dayU' THEN member_id END) + count(CASE WHEN r_type3 = 'r_3day' THEN member_id END) as user_7day,
